...
The webapp generalizes this with a single `putState(skPath, value)` function (and shared `pendingPut`/401-login-overlay retry flow) used by `toggleBreaker(key, value)`, `toggleAction(group, key, value)`, and `setDimmerStep(group, key, step)`. Like breakers, Lights & Pumps tiles are built once by `initLights()` (called at boot alongside `initBreakers()`) with click handlers bound to stable DOM nodes; `renderLights()` only updates `.light-dot` classes and dimmer-bar fill on each render cycle — do not rebuild tile `innerHTML` on every render, or click bindings would need constant rebinding and tapping could hit a stale node mid-rebuild.
