...
`BREAKER_MAP`/`ACTION_MAP`/`SHORTCUT_MAP` key format matches `toKey(name)` exactly (spaces → `_`, hyphens preserved) — except `SHORTCUT_MAP`, whose keys are already the Hub's own lowercase snake_case names (`navigation_light`, etc.), so no `toKey()` transform is applied there. The `name` field on each entry holds the original name for use in the MQTT `topic` field of the command payload. `ACTION_MAP` is namespaced by Signal K action group (`exterior_light`, `nacelle_lights`, `port_lights`, `stbd_lights`, `bilge_pumps`, `water_pump`) since the same base name (e.g. `reading_light`) is reused across `port_lights`/`stbd_lights` with different hardware addresses. The command `topic` field differs by category: breakers and shortcuts repeat the name twice (`breakers/children/{Name}/children/{Name}`, `contents/action_shortcut/children/{name}/children/{name}`), actions use it once (`actions/children/{group}/children/{name}`) — see `DEVICES.md` Command Reference. The shortcuts doubled-name format was confirmed by a live test publish against the real Hub (toggled the deck light on then back off via direct `mosquitto_pub`), not inferred from docs — don't assume it matches the `actions` single-name pattern just because both are "lights."
...
The webapp generalizes this with a single `putState(skPath, value)` function (and shared `pendingPut`/401-login-overlay retry flow) used by `toggleBreaker(key, value)`, `toggleAction(skPath, value)`, and `setDimmerStep(skPath, step)`. `skPath` is the full Signal K path relative to `sailsense.` (e.g. `actions.exterior_light.Submarine_light` or `contents.action_shortcut.navigation_light`) rather than a separate `group`+`key` pair — necessary because shortcut lights live under a structurally different prefix (`contents.action_shortcut.{key}`, no group nesting) than the rest of the lights/pumps (`actions.{group}.{key}`). Tiles carry this as a single `data-path` attribute. Like breakers, Lights & Pumps tiles are built once by `initLights()` (called at boot alongside `initBreakers()`) with click handlers bound to stable DOM nodes; `renderLights()` only updates `.light-dot` classes and dimmer-bar fill on each render cycle — do not rebuild tile `innerHTML` on every render, or click bindings would need constant rebinding and tapping could hit a stale node mid-rebuild.

### Shortcut lights (Navigation, Steaming, Anchor, Deck)

Four exterior lights — Navigation, Steaming, Anchor, Deck — live under a separate Hub subsystem, `contents/action_shortcut/children/{name}`, rather than the regular `actions/children/{group}/children/{name}` used by everything else in `LIGHT_GROUPS`/`PUMP_GROUPS`. `SHORTCUT_MAP` in `index.js` and `SHORTCUT_LIGHTS` in `public/index.html` hold the hardware addresses and display labels respectively; both were built from live retained MQTT data on the real Hub, not `DEVICES.md` (which only documents these as "(shortcut)" annotations in the Powernet Outputs table, without the `contents/action_shortcut/...` topic structure). `navigation_light` is dual-output (`{nPow:1,output:4}` + `{nPow:2,output:6}`), matching the doc's Rail 1/Rail 2 listing; the other three are single-output. The Hub's internal name for the Steaming light is `engine_light` (and its `DEVICES.md` label is "Engine light") — display it as "Steaming", not "Engine", per the actual hardware. Shortcut tiles render into the "Exterior Lights" section (appended after the `exterior_light` group's plain tiles in `initLights()`) since they are exterior lights, but are a structurally separate list from `LIGHT_GROUPS` — don't try to fold them into an `exterior_light`-group entry, their SK path and command topic shape are both different.

### Dimmable lights

The three `nacelle_lights` (Saloon, Saloon Indirect, Cockpit) support a 6-step dimmer in addition to on/off, per `DEVICES.md` "Dimmable Lights". `DIMMABLE_GROUPS` in `index.js` lists which `ACTION_MAP` groups get a `sailsense.actions.{group}.{key}.dimmer_step` put handler (currently just `nacelle_lights`) — the command payload uses `step` (0-5, clamped/rounded server-side) instead of `state`, but is otherwise the same `{nPow, output, topic}` shape as a toggle. `dimmer_step` was already forwarded to Signal K before this feature (`isLiveData()` in `index.js` already allow-lists the `/dimmer_step` suffix) — only the put handler was new.

On the webapp side, `LIGHT_GROUPS` entries can set `dimmable: true`; `initLights()` renders those with `buildDimmableTile()` (a `.light-card` containing a plain `.light-toggle` on/off row plus a `.dimmer-row` of 6 `.dimmer-step` buttons) instead of the plain `buildActionTile()`. `DIMMER_STEPS = [15, 25, 35, 45, 55, 100]` maps step index → brightness %, shown next to the bar. Dimmable cards render in their own `.dimmer-grid` (wider `minmax(300px, 1fr)` track) separate from the plain-tile `.light-grid` in the same section — mixing them in one `auto-fill` grid (e.g. via `grid-column: span 2` on the dimmer card) breaks track sizing on narrow/mobile widths where only one column fits; keep them in separate grids if you touch this again.

**Off-by-one between `dimmer_step` and the true dimmer index:** the Hub's `sailsense.actions.nacelle_lights.{key}.dimmer_step` status topic is **1-indexed** ("Level 1-6"), not a direct 0-based index into `DIMMER_STEPS`/`dimmer_table`. Confirmed by comparing it against the same light's `sailsense.powernet.device.powerail.{rail}.outputs.{output}.settings.value` (published alongside the Hub's own `dimmer_table` array, so unambiguously the true 0-based index) — the two were consistently offset by exactly 1 across all three lights. `DEVICES.md`'s command example (`"step": 3`) reads as a 0-based array index, but empirically the Hub's command handler expects the same 1-based numbering as its status topic — sending raw 0-based steps was silently ignoring/no-oping the lowest two levels. The fix keeps everything **else** in the codebase 0-based (`DIMMER_STEPS` indices, the `value` sent in the webapp's PUT body, `bindDimmerRow`/`updateDimmerRow`'s `step` params) and isolates the +1/-1 translation to exactly two places: `index.js`'s dimmer put handler adds 1 before publishing `step` to MQTT, and the webapp's `hubStepToIndex()` subtracts 1 when reading the retained `dimmer_step` value for display. If this ever needs re-verification against the real Hub, re-check `settings.value` vs `dimmer_step` for the same light — don't assume `DEVICES.md`'s example is correct without cross-checking.
...
- **Lights & Pumps** — four sections: Exterior Lights (includes the four `SHORTCUT_LIGHTS` — Navigation, Steaming, Anchor, Deck — alongside the regular `exterior_light` group), Interior Lights (nacelle + port/stbd cabin lights merged under one display heading), Bilge Pumps, Water Pumps
